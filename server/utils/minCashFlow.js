function calculateOptimizedSettlements(expenses, members) {
  const netBalances = {};
  ...
  activeExpenses.forEach(exp => {
    ...
    if (exp.splitType === 'exact' && exp.customSplits) {
      ...
      const customSplitsObj = exp.customSplits instanceof Map 
        ? Object.fromEntries(exp.customSplits) 
        : exp.customSplits;

      Object.entries(customSplitsObj).forEach(([person, personShare]) => {
        const share = Number(personShare) || 0;
...
module.exports = { calculateOptimizedSettlements };